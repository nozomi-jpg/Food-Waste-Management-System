...
const db = mysql.createConnection({
  ...
  user: "if0_35929223",
  password: "CMix5A8dyVF",
  database: "if0_35929223_foodwastedb",
});
...
app.get("/types", (req, res) => {
  const q =
    "SELECT `Type_ID`, `Type_name`, `Is_perishable` FROM type WHERE User_Id = ?";
...
app.get("/dayWaste", (req, res) => {
  const q =
    "SELECT `Name_inventory`, `Kg_waste`, `Pcs_waste`, `Price` FROM `waste` INNER JOIN inventory ON waste.Inventory_ID = inventory.Inventory_ID WHERE waste.User_id = ? AND Date_waste = CURRENT_DATE";
  const userID = 1000; //to be changed
...
app.get("/expiringToday", (req, res) => {
  const q =
    "SELECT `Name_inventory`FROM inventory WHERE User_Id = ? AND Expiration_date = CURRENT_DATE";
  const userID = 1000; //to be changed
...
app.get("/expiringWeek", (req, res) => {
  const q =
    "SELECT `Name_inventory`FROM inventory WHERE User_Id = ? AND Expiration_date BETWEEN ? AND ?";
  const userID = 1000; //to be changed
...
app.get("/mostWasted", (req, res) => {
  const q =
    "SELECT `Name_inventory`, `Price` FROM `waste` INNER JOIN inventory ON waste.Inventory_ID = inventory.Inventory_ID WHERE waste.User_id = ?";
  const userID = 1000; //to be changed
...
app.get("/expiredStats", (req, res) => {
  const q =
    "SELECT expired.Price FROM `expired` INNER JOIN inventory ON expired.Inventory_ID = inventory.Inventory_ID WHERE inventory.User_id = ?";
  const userID = 1000; //to be changed
  db.query(q, [userID], (err, data) => {
    if (err) console.log(err);
    return res.json(data);
...
app.get("/monthlyReport", (req, res) => {
  const q =
    "SELECT SUM(Kg_waste), SUM(Price), MONTH(Date_waste) FROM waste INNER JOIN inventory ON waste.Inventory_ID = inventory.Inventory_ID GROUP BY MONTH(Date_waste)";
...
app.get("/ingredients", (req, res) => {
  const q =
    "SELECT `Inventory_ID`, `Name_inventory`, `Type_name`, `Pcs_inventory`, `Kg_inventory`, `Price`, DATE_FORMAT(`Expiration_date`, '%Y-%m-%d') AS `Expiration_date`, inventory.Type_ID FROM inventory INNER JOIN type ON inventory.Type_ID = type.Type_ID WHERE inventory.User_ID = ?";
  const userID = 1000;
  db.query(q, userID, (err, data) => {
    if (err) return res.json(err);
    return res.json(data);
...
app.get("/inventory/expiration-table", async (req, res) => {
  try {
    ...
    const i =
      "INSERT INTO expired (`Price`, `Inventory_ID`) SELECT `Price`, `Inventory_ID` FROM inventory WHERE `Expiration_date` < NOW() AND `Inventory_ID` NOT IN (SELECT `Inventory_ID` FROM expired)";
    await db.query(i);

    // Then, perform the SELECT query
    const q =
      "SELECT `Exp_ID`, `Name_inventory`, `Type_name`, `Pcs_inventory`, `Kg_inventory`, expired.Price, DATE_FORMAT(`Expiration_date`, '%Y-%m-%d') AS `Expiration_date` FROM expired LEFT JOIN inventory ON expired.Inventory_ID = inventory.Inventory_ID LEFT JOIN type ON inventory.Type_ID = type.Type_ID";
    const userID = 1000;
    db.query(q, (err, data) => {
      if (err) return res.json(err);
      return res.json(data);
...
app.post("/addInventory", (req, res) => {
  ...
  const q1 =
    "SELECT * FROM `inventory` WHERE `User_id` = ? AND `Name_inventory` = ?";
  const q2 =
    "INSERT INTO `inventory` (`User_id`, `Type_ID`, `Name_inventory`,`Kg_inventory`, `Pcs_inventory`, `Price`, `Expiration_date`) VALUES (?)";
  const userID = 1000;
...
app.post("/addType", (req, res) => {
  const q1 =
    "SELECT `Type_name` FROM `type` WHERE `Type_name` = ? AND User_id = ? ";
  const q2 =
    "INSERT INTO `type` (`User_id`,`Is_perishable`,`Type_name`) VALUES (?)";
  const userID = 1000;
  const isPerishable = req.body.current_ISPERISHABLE == false ? 0 : 1;

  const values = [userID, isPerishable, req.body.current_TYPENAME];
